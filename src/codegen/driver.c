...
bool jag_codegen_compile_native(const char *c_filename, const char *bin_filename, JagCodegenOptions *opts) {
    ...
    snprintf(cmd, sizeof(cmd),
             ...
             "cl /nologo /O2 /std:c17 /I\"%s\" /I. %s \"%s\\Debug\\jaguar_runtime.lib\" ws2_32.lib /Fe:\"%s\" > NUL 2>&1 || "
             "cl /nologo /O2 /std:c17 /I\"%s\" /I. %s \"%s\\Release\\jaguar_runtime.lib\" ws2_32.lib /Fe:\"%s\" > NUL 2>&1 || "
             "gcc -O2 -std=c17 -I\"%s\" -I. -L\"%s\" -L\"%s\\Debug\" -L\"%s\\Release\" %s -ljaguar_runtime -lws2_32 -o \"%s\" > NUL 2>&1 || "
             "clang -O2 -std=c17 -I\"%s\" -I. -L\"%s\" -L\"%s\\Debug\" -L\"%s\\Release\" %s -ljaguar_runtime -lws2_32 -o \"%s\" > NUL 2>&1",
