...
bool jag_codegen_generate_c(JagASTNode *ast, const char *output_c_filename, JagCodegenOptions *opts) {
    ...
    // Self-contained runtime type declarations
    fprintf(f, "typedef enum { JAG_VAL_NUM, JAG_VAL_DECIMAL, JAG_VAL_BOOL, JAG_VAL_STRING, JAG_VAL_ARRAY, JAG_VAL_NULL } JagValKind;\n");
    fprintf(f, "typedef struct JagVal JagVal;\n");
    fprintf(f, "typedef struct JagArray JagArray;\n");
    fprintf(f, "struct JagArray { JagVal *items; size_t count; size_t capacity; };\n");
    fprintf(f, "struct JagVal { JagValKind kind; union { int64_t num; double decimal; bool boolean; char *string; JagArray *array; } as; };\n\n");

    // Runtime function prototypes
    fprintf(f, "JagVal jag_val_num(int64_t n);\n");
    fprintf(f, "JagVal jag_val_decimal(double d);\n");
    fprintf(f, "JagVal jag_val_bool(bool b);\n");
    fprintf(f, "JagVal jag_val_string(const char *str);\n");
    fprintf(f, "JagVal jag_val_array(JagArray *arr);\n");
    fprintf(f, "JagVal jag_val_null(void);\n");
    fprintf(f, "bool jag_val_equals(JagVal a, JagVal b);\n");
    fprintf(f, "char *jag_val_to_string(JagVal v);\n");
    fprintf(f, "char *jag_str_concat(const char *a, const char *b);\n");
    fprintf(f, "JagArray *jag_array_create(size_t initial_cap);\n");
    fprintf(f, "void jag_array_append(JagArray *arr, JagVal val);\n");
    fprintf(f, "void jag_array_sort(JagArray *arr);\n");
    fprintf(f, "JagVal jag_array_get(JagArray *arr, size_t index);\n");
    fprintf(f, "void jag_live_on_val(JagVal v);\n");
    fprintf(f, "void jag_live_log(const char *msg);\n");
    fprintf(f, "char *jag_live_in(void);\n");
    fprintf(f, "void jag_live_deg(const char *type_name, JagVal expected, JagVal actual);\n");
    fprintf(f, "typedef struct { FILE *fp; char *path; } JagFile;\n");
    fprintf(f, "JagFile *jag_file_open(const char *path, const char *mode);\n");
    fprintf(f, "char *jag_file_read(JagFile *file);\n");
    fprintf(f, "void jag_file_close(JagFile *file);\n");
    fprintf(f, "typedef struct { char *path; void *dir_handle; } JagDir;\n");
    fprintf(f, "JagDir *jag_dir_on(const char *path);\n");
    fprintf(f, "JagArray *jag_dir_read(JagDir *dir);\n");
    fprintf(f, "void jag_dir_close(JagDir *dir);\n\n");
